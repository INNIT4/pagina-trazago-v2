...
const empty: FormData = { name: "", phoneNumber: "", description: "", category: "Emergencia" };
...
export default function EmergenciaEditorPage() {
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, "emergency_contacts", id)).then((snap) => {
      if (snap.exists()) {
        const d = snap.data();
        setForm({ name: d.name ?? "", phoneNumber: d.phoneNumber ?? "", description: d.description ?? "", category: d.category ?? "Emergencia" });
      }
      setLoading(false);
    });
  }, [id, isNew]);
    ...
    <>
      ...
      <div className="admin-form-section" style={{ maxWidth: 480 }}>
        ...
        <div className="admin-form">
          ...
          <div className="admin-field"><label>Categoría</label><input value={form.category} onChange={(e) => set("category", e.target.value)} /></div>
        </div>
      </div>
    </>
  );
