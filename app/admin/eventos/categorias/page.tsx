...
export default function EventoCategoriesPage() {
  ...
  async function load() {
    setLoading(true);
    const snap = await getDocs(query(collection(db, "event_categories"), orderBy("order")));
  ...
  async function handleSave() {
    ...
    try {
      if (editId) {
        await setDoc(doc(db, "event_categories", editId), data);
      } else {
        await addDoc(collection(db, "event_categories"), data);
      }
      cancelEdit();
      await load();
    } catch {
    ...
    <>
      <div className="admin-topbar">
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Link href="/admin/eventos" className="admin-btn admin-btn-ghost admin-btn-sm">← Eventos</Link>
          <h1 className="admin-title">Categorías de eventos</h1>
        </div>
      </div>
          ...
          <div className="admin-form-row">
            <div className="admin-field">
              ...
              <input
                ...
                placeholder="Festival, Cultural, Religioso..."
              />
            </div>
            <div className="admin-field">
              <label>Orden</label>
