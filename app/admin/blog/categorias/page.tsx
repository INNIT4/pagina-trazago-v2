...
export default function BlogCategoriesPage() {
  ...
  async function load() {
    setLoading(true);
    const snap = await getDocs(query(collection(db, "blog_categories"), orderBy("order")));
  ...
  async function handleSave() {
    ...
    try {
      if (editId) {
        await setDoc(doc(db, "blog_categories", editId), data);
      } else {
        await addDoc(collection(db, "blog_categories"), data);
      }
      cancelEdit();
      await load();
    } catch {
    ...
    <>
      <div className="admin-topbar">
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Link href="/admin/blog" className="admin-btn admin-btn-ghost admin-btn-sm">← Blog</Link>
          <h1 className="admin-title">Categorías del blog</h1>
        </div>
      </div>
          ...
          <div className="admin-form-row">
            <div className="admin-field">
              ...
              <input
                ...
                placeholder="Crónica, Guía, Historia..."
              />
            </div>
            <div className="admin-field">
              <label>Orden</label>
